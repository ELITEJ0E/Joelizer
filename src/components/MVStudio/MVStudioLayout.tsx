...
import { LyricsVideoLayout } from '../LyricsStudio/LyricsVideoLayout';
...
import { Layers, Sliders, Play, Film, X, Sparkles, Music } from 'lucide-react';
...
export function MVStudioLayout() {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#030304] text-slate-300 font-sans select-none overflow-hidden relative">
      ...
      <div className="bg-[#08080a] border-b border-white/10 px-3 py-1.5 flex items-center justify-between shrink-0 z-40">
        <div className="flex items-center gap-1 bg-black/60 p-1 rounded-xl border border-white/10 shadow-inner">
          <button
            onClick={() => setVideoMode('lyrics-video')}
          ...
          >
            <Sparkles size={13} className={videoMode === 'lyrics-video' ? 'text-amber-500' : ''} />
            <span>Lyrics Video Studio</span>
          </button>

          <button
            onClick={() => setVideoMode('music-video')}
            className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer ${
              videoMode === 'music-video'
                ? 'bg-white text-black font-extrabold shadow-md'
                : 'text-slate-400 hover:text-white hover:bg-white/5'
            }`}
          >
            <Film size={13} className={videoMode === 'music-video' ? 'text-sky-500' : ''} />
        ...
        <div className="hidden sm:flex items-center gap-2">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">
            {videoMode === 'lyrics-video' ? 'Mureka-Style Sync Engine' : 'Auto Editor & Timeline'}
          </span>
        </div>
      </div>
